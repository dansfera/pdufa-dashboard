...
// EJS templating — restoration-specific views directory
app.set('view engine', 'ejs');
app.set('views', path.join(__dirname, 'views'));

// Static files served under /restoration/css/...
// (handled from the parent server via express.static on /restoration)
...
const baseUrl = process.env.RESTORATION_BASE_URL || 'https://sunbeltrestorationguide.com';

function commonData() {
  return { cities, services, articles, analyticsSlug, baseUrl, ga4Id: process.env.GA4_RESTORATION_ID || '' };
}
...
app.get('/', (req, res) => {
  res.render('pages/home', {
    title: 'Sunbelt Restoration Guide — Water Damage Restoration Information',
    metaDescription: 'Expert water damage restoration guides for Sunbelt cities. Find local restoration info, costs, insurance tips, and what to do after a flood or water damage event in Yuma, Tulsa, Greenville, Tucson, and Kansas City.',
    canonicalUrl: '/',
    ...commonData()
  });
});
...
app.get('/about', (req, res) => {
  res.render('pages/about', {
    title: 'About Sunbelt Restoration Guide — Trusted Water Damage Information',
    metaDescription: 'Learn about Sunbelt Restoration Guide, your trusted source for water damage restoration information across the Sunbelt region. No sales pitches, no referrals — just honest information.',
    canonicalUrl: '/about',
    ...commonData()
  });
});
...
app.get('/disclosure', (req, res) => {
  res.render('pages/disclosure', {
    title: 'Affiliate Disclosure — Sunbelt Restoration Guide',
    metaDescription: 'Affiliate disclosure for Sunbelt Restoration Guide.',
    canonicalUrl: '/restoration/disclosure/',
    ...commonData()
  });
});
...
app.get('/advertise', (req, res) => {
  ...
  res.render('pages/advertise', {
    title: 'Advertise With Us — Sunbelt Restoration Guide',
    metaDescription: 'Reach local homeowners searching for water damage restoration in Yuma, Tucson, Tulsa, Greenville, and Kansas City. Banner ads, sponsored listings, and featured placements available.',
    canonicalUrl: '/restoration/advertise',
    formSuccess,
    ...commonData()
  });
});
...
app.post('/advertise', async (req, res) => {
  ...
  if (!name || !business_name || !email || !message) {
    return res.render('pages/advertise', {
      title: 'Advertise With Us — Sunbelt Restoration Guide',
      metaDescription: 'Reach local homeowners searching for water damage restoration services.',
      canonicalUrl: '/restoration/advertise',
  ...
  try {
    ...
    if (pool) {
      await pool.query(
        ...
        ['restoration', name, business_name, email, message]
      );
    }
  } catch (err) {
  ...
  try {
    ...
    await sendAdInquiryEmail({ site: 'restoration', name, businessName: business_name, email, message });
  } catch (err) {
    console.error('[Advertise] Email error:', err.message);
  }

  res.redirect('/restoration/advertise?success=1');
});
...
app.get('/:citySlug', (req, res, next) => {
  ...
  res.render('pages/city', {
    title: `Water Damage Restoration in ${city.fullName} — Local Guide | Sunbelt Restoration Guide`,
...
app.get('/services/:serviceSlug', (req, res, next) => {
  ...
  res.render('pages/service', {
    title: `${service.name} — Guide, Costs & What to Do | Sunbelt Restoration Guide`,
...
app.use((req, res) => {
  res.status(404).render('pages/404', {
    title: 'Page Not Found — Sunbelt Restoration Guide',
